...
class SendEmail{
    ...
    /*邮件用户名*/
    public $mailUser = 'fanlierp';

    /*邮件密码*/
    public $mailPwd = 'fanlierp@123';

    /*邮件服务器地址*/
    public $server = '192.168.100.150';
    ...
    /*邮件发送者email,用于显示给接收者*/
    public $senderMail = 'fanlierp@fanli.com';

    /*发用者名称*/
    public $senderName = 'fanlierp';
...
//                }
    ...
    */
    ...
    public function mail_log($email,$title,$error_info=""){
        return ;
        $mail_send_log = D('MailSendLog');
        $email = serialize($email);
        $data = array('email'=>$email,'subject'=>$title,'error_info'=>$error_info);
        $mail_send_log->add($data);
    }
    ...
    protected function mimeHeader($msg,$to,$title){
        ...
        if($this->attachMent!=null){
            ...
            foreach($this->attachMent as $k=>$filename){
                ...
                $filename = $this->get_basename($filename);

                $headers[] = "--" . $boundary;
                $headers[] = "Content-type: ".$mimetype.";name=\"=?".$this->charset."?B?". base64_encode($filename).'?="' ;
                $headers[] = "Content-disposition: attachment; name=\"=?".$this->charset."?B?". base64_encode($filename).'?="';
                $headers[] = 'Content-Transfer-Encoding: base64'.$this->stop;
